...
Проверьте наличие сетевого интерфейса `docker0` с помощью команды 
...
_Примечание: на Ubuntu аналогом этой команды будет `ip a`._
...
Используйте инструменты и команды для загрузки образов Docker из терминала виртуальной машины. Начните с образа bash, который предназначен для запуска на его основе контейнеров, для последующей работы в командной оболочке bash.
...
где:
- `image ls` - показать загруженные и размещённые в системе образы Docker.
...
Важно понимать, что функционал образа bash ограничен его непосредственным назначением - работой в командной оболочке bash. Этого достаточно на текущем этапе знакомства с контейнерами Docker.
...
Как не запутаться и понять, что вы находитесь в контейнере или в терминале виртуальной машины? Попробуем разобраться с этим вопросом подробнее. 
...
где:
- `ps` - показать список активных контейнеров.
...
Вы остановили работу контейнера. Что с ним произошло на самом деле?
...
где:
- `ps -a` - показать список всех контейнеров.
...
где:
- `attach` - подключиться к активному контейнеру.
...
где:
- `rm` - удалить неактивный контейнер.
