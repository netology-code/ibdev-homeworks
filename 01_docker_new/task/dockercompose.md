...
- отработаете базовые команды для управления связкой контейнеров,
- замените в одном из контейнеров рабочий файл своим подготовленным экземпляром,
...
2. Для этого проекта необходимо подготовить два файла в рабочем каталоге проекта. Для этого Перейдите в правый терминал и создайте пустые файлы `index.html` и `docker-compose.yml` соответствующей командой. Проверьте их наличие в рабочем каталоге.
...
Вставьте этот скрипт в окно текстового редактора открытого файла. Вместо присутствующего в файле ФИО наберите свои данные. 
...
Перейдите в правый терминал и проверьте наличие контейнеров и их состояние. Вы увидете два неактивных контейнера.
...
Проверьте наличие контейнеров в основной системе и убедитесь, что они отсутствуют и были удалены после остановки контейнеров через docker-compose.

Отлично! Вы успешно завершили работу с Docker.
...
- заменили в одном из контейнеров рабочий файл своим подготовленным экземпляром,
