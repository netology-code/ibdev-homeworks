...
В рамках этого задания мы посмотрим, как настроена по умолчанию аутентификация в Docker образе PostgreSQL v12.
...
1. Для помещения shell (bash) в контейнер используйте команду `docker-compose exec postgres /bin/bash`.
...
Изучите файл `/var/lib/postgresql/data/pg_hba.conf` и ответьте на следующе вопросы, используя команду получения shell:
...
1. Верно ли следующее утверждение: пароль роли `app` хранится в виде `функция_хеширования password` (пароль хранится в поле `rolpassword`)? Если неверно, то приведите описание алгоритма, который используется для хранения хеша.
...
В рамках этого задания мы изучим механизмы управления пользователями и ПРД, реализованные в рамках СУБД PostgreSQL.
...
1\. Получим psql для пользователя `reader`: `docker-compose exec postgres psql -U reader -d test` (флаг -p означает, что подключаемся с паролем)
...
К этим материалам мы вернёмся в процессе прохождения курса, но вы уже сейчас можете ознакомиться с ними:
