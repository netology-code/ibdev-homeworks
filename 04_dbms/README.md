...
Эти домашние задания — лабораторные работы, в рамках которых вы по инструкциям выполните действия.
...
1. Верно ли утверждение: пароль роли `app` хранится в виде `функция_хеширования password` (пароль хранится в поле `rolpassword`)? Если неверно, то приведите описание алгоритма, который используется для хранения хеша.
...
Изучите разделы документации, касающиеся управления пользователями и ПРД:
...
В простейшем случае есть разделение на структуры, вроде БД, или таблицы и данные, вроде строк в таблице. БД и таблицы создаются или удаляются с помощью запросов `CREATE` или `DROP`, а вот данные можно вставлять или удалять с помощью запросов `INSERT` или `DELETE`.
...
Команда `\dt` показывает таблицы, существующие в текущей БД, к которой вы подключены.
...
8\. Предоставьте права роли `reader` на чтение содержимого таблицы `records` `GRANT SELECT ON records TO reader`:
```postgresql
...
3\. Удостоверьтесь, что остальные действия запрещены:
```postgresql
